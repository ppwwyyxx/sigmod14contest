...
bool cmp_d(int a, int b) {
    return (Data::birthday[a] > Data::birthday[b] || (Data::birthday[a] == Data::birthday[b] && a < b));
}

void Query2Handler::add_query(int k, int d) {
    queries.push_back(Query2(k, d, (int) queries.size()));
}
...
vector<int> find_ans(int k) {
    ...
    for (int j=0; j<k; j++)
        heap.insert(tmp[j].tag_id, tmp[j].range);
    return ans_now;
}
...
void Query2Handler::work() {
    ...
    for (int i=0; i<(int) Data::person_in_tags.size(); i++) {
        sort(Data::person_in_tags[i].begin(), Data::person_in_tags[i].end(), cmp_d);
    ...
    for (int i=0; i<Data::nperson && queryP<(int) queries.size(); i++) {
        ...
        //answer queries
        for (; queries[queryP].d > Data::birthday[person_now] && queryP<(int) queries.size(); queryP ++)
            ans.push_back(find_ans(queries[queryP].k));
        if (queryP == (int) queries.size()) break;

        //new person in tag
        for (auto j=Data::tags[person_now].begin(); j!=Data::tags[person_now].end(); j++) {
            int tag_now = *j;
        ...
        for (int j=0; j<(int) myfriends[person_now].size(); j++) {
            ...
            for (auto k=Data::tags[friend_now].begin(); k!=Data::tags[friend_now].end(); k++) {
                int tag_now = *k;
                if (Data::tags[person_now].find(tag_now) == Data::tags[person_now].end()) continue;
				int p = myhash[tag_now][friend_now];
                int set_now = getf(p, f[tag_now]);
                if (set_now == f[tag_now][(int) f[tag_now].size()-1]) continue;
                sum[tag_now][(int) f[tag_now].size()-1] += sum[tag_now][set_now];
